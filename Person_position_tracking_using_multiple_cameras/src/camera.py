...
# def recognize_faces_from_frame(frame, embeddings_file):
#     # Save the frame temporarily as an image file
#     temp_image_path = f"{config.DATA_PATH}/temp/temp_frame.jpg"
#     cv2.imwrite(temp_image_path, frame)

#     # Use the recognize_faces function to process the frame
#     results = recognize_faces(temp_image_path, embeddings_file)
...
if __name__ == "__main__":
    ...
    # camera2 = Camera(camera_no=1, location="Entrance", zone_no=2, zone_name="Main Entrance",interval=config.FRAME_CAPTURE_INTERVAL,source=f"{config.DATA_PATH}/inputs/videos/Two_ankers.mp4")
    # thread1 = threading.Thread(target=run_camera, args=(camera1,))
    # thread2 = threading.Thread(target=run_camera, args=(camera2,))
